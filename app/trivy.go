...
func (s TrivyScanner) Scan(scanType, scanTarget, severity string, pipelineMode bool) Scan {
	...
	switch scanType {
	case "config":
		return s.run(fmt.Sprintf("trivy config %s %s %s", severityOpt, outputOpt, scanTarget), scanType, scanTarget, pipelineMode)
	case "files":
		return s.run(fmt.Sprintf("trivy filesystem %s %s %s", severityOpt, outputOpt, scanTarget), scanType, scanTarget, pipelineMode)
	case "image":
		return s.run(fmt.Sprintf("trivy image %s %s %s", severityOpt, outputOpt, scanTarget), scanType, scanTarget, pipelineMode)
	default:
		return Scan{}
	}
}

func (s TrivyScanner) run(cmdline, scanType, scanTarget string, pipelineMode bool) Scan {
	beginTime := time.Now()
